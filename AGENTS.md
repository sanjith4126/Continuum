...
append-only `ledger_event` table records important changes for the audit trail.
Historical P&L is calculated from the effective timestamps on operational
financial records; it is not rebuilt by replaying the ledger. This is a
relational transactional design with an audit log, not full event sourcing.
...
- Secrets in env vars, never in code. `DATABASE_URL`, `GROQ_API_KEY_CONSULTANT`,
  ...
  `GROQ_API_KEY_ASSISTANT_FALLBACK`, `RESEND_API_KEY`, and `RESEND_FROM`.

## Conventions
- Every important write also appends a `ledger_event` (event_type, entity,
  batch_id, and signed amount for money) for traceability. As-of P&L uses the
  effective timestamps on `invoice_line`, `expense`, and `trainer_payment`.
...
recalculates the P&L as of last week; one click walks the Acme lead to its margin; a
...
## Key files
- `db/schema.sql` — relational schema, audit ledger, reporting views (`batch_pnl`,
