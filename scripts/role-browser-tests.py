...
def navigate(page, route):
 for attempt in range(2):
  try:return page.goto(BASE+route,wait_until='domcontentloaded',timeout=60000)
  except Exception:
   if attempt:raise
...
with sync_playwright() as p:
 ...
 for route in routes:
  navigate(page,route);assert '/login' in page.url,route
 ...
 for a in accounts:
  ...
  navigate(page,allowed[a['role']][0])
  ...
  sign_out(page);page.wait_for_url('**/login',timeout=30000)
