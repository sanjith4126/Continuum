...
OUT=Path('audit-results');BASE='http://127.0.0.1:3000'
...
with sync_playwright() as p:
 ...
 for endpoint in ['assistant','consultant']:
  response=anon.request.post(BASE+'/api/'+endpoint,data={'question':'balance'});assert response.status==401
 ...
 for a in accounts:
  ...
  for endpoint in ['assistant','consultant']:
   ...
   response=context.request.post(BASE+'/api/'+endpoint,data={'question':''})
   assert response.status==(400 if permitted else 403),(a['role'],endpoint,response.status,response.text())
  if a['role']=='student':
   response=context.request.post(BASE+'/api/assistant',data={'question':'balance','studentId':'00000000-0000-0000-0000-0000000000a5'})
  ...
  context.storage_state(path=str(OUT/(a['email'].split('@')[0]+'-session.json')))
  page.get_by_role('button',name='Sign out',exact=True).click();page.wait_for_url('**/login',timeout=30000)
  response=context.request.post(BASE+'/api/assistant',data={'question':''});assert response.status==401
